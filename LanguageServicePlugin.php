<?php
/**
 * Language Service Dependency Injector
 *
 * @package   Molajo
 ...
 * @since     1.0
 */
class LanguageServicePlugin extends Injector implements InjectorInterface
{
    /**
    ...
    public function onAfterServiceInstantiate()
    {
        ...
        if ($language == '') {
            ...
            $registry = $this->service_instance->get('registry');

            foreach ($registry as $entry) {

                if ($entry->tag == $language) {
                    $this->service_instance->set('registry', $entry);
                    break;
                }
            }
        }
        ...
        $this->service_instance->set(
            ...
            $this->application_instance->get('profiler_collect_missing_language_strings')
        );

        if (Services::User()->get('administrator') === 1) {
            $this->service_instance->set('insert_missing_strings', 1);
        }
    ...
    protected function setInstalledLanguages()
    {
        ...
        $this->service_instance->set('installed', $tagArray);
        $this->service_instance->set('registry', $languageList);
    ...
    protected function setLanguage()
    {
        ...
        if ($language == '') {
            ...
            if (in_array($language, $this->get('installed'))) {
                $this->service_instance->set('language', $language);

                return;
            }
        }

        if (count($this->service_instance->get('installed')) == 1) {
            $languages = $this->service_instance->get('installed');
            $this->service_instance->set('language', $languages[0]);
        ...
        $language = $this->application_instance->get('language');
        if (in_array($language, $this->get('installed'))) {
            $this->service_instance->set('language', $language);
        ...
        if (isset($_SERVER['HTTP_ACCEPT_LANGUAGE'])) {
            ...
            if (count($browserLanguages) > 0) {
                foreach ($browserLanguages as $language) {
                    if (in_array($language, $this->service_instance->get('installed'))) {
                        $this->service_instance->set('language', $language);
    ...
    protected function setLanguageStrings()
    {
        ...
        $this->service_instance->set('strings', $strings);
